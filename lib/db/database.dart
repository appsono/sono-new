...
class SonoDatabase extends _$SonoDatabase {
  ...
  Future<int> getOrCreateArtist(String name) async {
    final existing = await (select(artists)
  ...
  /// ==== Albums ====
  Future<int> getOrCreateAlbum(String title, int artistId, Uint8List? cover) async {
  ...
  Future<bool> songExists(String path) async {
    final count = await (select(songs)..where((s) => s.path.equals(path)))
        .getSingleOrNull();
    return count != null;
  }
